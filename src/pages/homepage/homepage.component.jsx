import React,{ useEffect, useRef, useState } from 'react'
import { connect } from "react-redux"
import { Link } from "react-router-dom";

import "./homepage.styles.css"
import { updatePostAsync, updateHeartAsync, updateHeartBooleanAsync, updateHeartBooleanOnRedux } from "../../redux/posts/posts.actions"

function Homepage({ posts, updatePostAsync, updateHeartAsync, 
        heartBoolean, currentUser, updateHeartBooleanAsync, updateHeartBooleanOnRedux, waitTime }) {
    const [spanHeartId, setSpanHeartId] = useState("");

    const heartIconRef = useRef();

    if(posts){
        heartIconRef.current = new Array(posts.length)
    }
    useEffect(() => {
        updatePostAsync();
    }, [])

    useEffect(() => {
        if(!heartBoolean) return;
        heartBoolean.map(heart => {
            if(heart.heartBoolean){
                setSpanHeartId(heart.postId)
            }
        })
    }, [heartBoolean])

    useEffect(() => {
        if(!heartIconRef.current) return;
        if(!posts) return;
        const selectedHeartRef = heartIconRef.current.filter(icon => icon.id === spanHeartId);

        if(!selectedHeartRef[0]) return;

        heartBoolean.forEach(heart => {
            if(heart.heartBoolean && heart.postId === spanHeartId){
                selectedHeartRef[0].classList.add("heart-icon-active");
            }else if(!heart.heartBoolean && heart.postId === spanHeartId){
                selectedHeartRef[0].classList.remove("heart-icon-active");
            }
        })
                        
    }, [ heartBoolean,spanHeartId])

    useEffect(() => {
        if(!currentUser) return;
        if(!posts) return;
        posts.forEach(post => updateHeartBooleanAsync(post.id, currentUser.uid))
    }, [posts])

    return (
        <main className="center-homepage col-md-7">
           <div className="container-fluid">
           {
               posts && posts.map((post, idx) => (
                <div key={post.id} className="post-container my-5 p-3">
                    <div className="top-post-head">
                        <div className="top-left-head">
                            <div className="post-img-container">
                                <img src='data:image/png;base64,' alt="default picture"/>
                            </div>
                            <h5 className="display-4 user-name ml-2"> {post.displayName} </h5>
                        </div>
                        <div className="top-right-head text-right">
                            <p className="date"> {post.date} </p>
                            <p className="time"> {post.time} </p>
                        </div>
                    </div>
                    <h4 className="post-title mt-2"> {post.title} </h4>
                    <div className="bottom-post mt-2">
                        <div className="post-content">
                            <p> {post.content} </p>
                        </div>
                        <div className="link-container mt-2">
                        {
                            post.link && <p>Link : <a href={ post.link } target="_blank"> {post.link} </a></p>
                        }
                        </div>
                        <div className="heart-container mt-2">
                            <span id={post.id} ref={el => heartIconRef.current[idx] = el} onClick={() =>{
                                updateHeartAsync(post.id, heartBoolean[idx].heartBoolean, currentUser.uid);
                                updateHeartBooleanOnRedux(post.id);
                                setSpanHeartId(post.id);
                            }} className="heart"> &#10084; </span><span className="heart-number ml-2"> {post.heart} </span>
                        </div>
                        <div className="comment-container">
                            <Link to="/">
                                <h5 className="display-4">Add a comment</h5>
                            </Link>
                        </div>
                    </div>
                </div>
               ))
               
           }
           </div>
        </main>
    )
}

const mapStateToProps = state => ({
    currentUser: state.user.currentUser,
    posts: state.posts.posts,
    heartBoolean: state.posts.heartBoolean,
    waitTime: state.posts.waitTime
})

const mapDispatchToProps = (dispatch) => ({
    updatePostAsync: posts => dispatch(updatePostAsync(posts)),
    updateHeartAsync: (postId, heartBoolean, userAuthId) => dispatch(updateHeartAsync(postId, heartBoolean,userAuthId)),
    updateHeartBooleanAsync: (postId, userAuthId) => dispatch(updateHeartBooleanAsync(postId, userAuthId)),
    updateHeartBooleanOnRedux: postId => dispatch(updateHeartBooleanOnRedux(postId))
})

export default connect(mapStateToProps, mapDispatchToProps)(Homepage)