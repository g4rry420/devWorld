import React,{ useEffect } from 'react'
import { connect } from "react-redux"
import { createStructuredSelector } from "reselect"

import "./notifications.styles.css"
import { userProfilesAsync } from "../../redux/user/user.actions"
import { updatePostAsync } from "../../redux/posts/posts.actions"
import { selectNotification } from "../../redux/notifications/notification.selectors"

function Notifications({ notification, userProfilesAsync, rightHomepageRef }) {
    useEffect(() => {
        userProfilesAsync();
        updatePostAsync()
    }, [userProfilesAsync])

    return (    
        <aside ref={rightHomepageRef} className="right-homepage col-md-3">
            <h2>Notifications</h2>
            {
                notification && notification.sort((a, b) =>  b.createdAt.seconds - a.createdAt.seconds ).map(noti => (
                    <div key={noti.uid ? noti.uid : noti.id} className="noti-container my-3 p-2">
                        <div className="top-noti-head">
                            <div></div>
                            <div className="top-noti-right-head text-right">
                                <p className="date"> {noti.date} </p>
                                <p className="time"> {noti.time} </p>
                            </div>
                        </div>
                        <div className="noti-left-head">
                            <div className="noti-img-container">
                                <img src='data:image/png;base64,' alt="default "/>
                            </div>
                            {
                                noti.uid ? <h5 className="display-4 noti-name ml-2">  {noti.displayName}  has joined the party</h5> :
                                    noti.id ? <h5 className="display-4 noti-name ml-2">  {noti.displayName} added a post</h5> : null
                            }
                        </div>
                    </div>
                ))
                }
        </aside>
    )
}

const mapStateToProps = createStructuredSelector ({
    notification: selectNotification
})

const mapDispatchToProps = dispatch => ({
    userProfilesAsync: () => dispatch(userProfilesAsync()),
    updatePostAsync: () => dispatch(updatePostAsync())
})

export default connect(mapStateToProps, mapDispatchToProps)(Notifications)